...
const IndexScreen = () => {
  ...
  useEffect(() => {
    const fetchInventoryData = async () => {
      try {
        const response = await axios.get("http://192.168.1.13:3000/");
  ...
  const deleteItem = async (itemId) => {
    try {
      const response = await axios.delete(`http://192.168.1.13:3000/delete/${itemId}`);
  ...
  const handleSearch = () => {
    axios
     .get(`http://192.168.1.13:3000/search?q=${itemName}`)
     .then((response) => {
  ...
  const handleEdit = () => {
    ...
    axios
      .put(`http://192.168.1.13:3000/edit/${editedItem._id}`, {
        itemName: editedItem.itemName,
        quantity: editedItem.quantity,
        expiredDate: expiredDate, // Use the original expiration date
  ...
  const calculateExpiration = () => {
    const diff = moment(expiredDate, 'ddd MMM DD YYYY').diff(moment(), 'days');
    if (diff < 0) {
      setExpired(true); // Set expired to true if the item is expired
    } else {
      setExpired(false);
    }
  };
  ...
  const renderItem = ({ item, index, itemId }) => {
    ...
    console.log(item.manufactors)
    return (
      <View style={styles.row}>
        <Text style={[styles.cell, { width: 100 }]}>{item.itemName}</Text>
        <Text style={[styles.cell, { width: 90, paddingLeft: 10,}]}>{item.quantity}</Text>
        <Text style={[styles.cell,{ width: 110, color: isExpired ? "red" : "green" }]}>
        {moment(item.expiredDate, "YYYY-MM-DDTHH:mm:ssZ").format("ddd MMM DD YYYY")}
         </Text>     
         <Text style={[styles.cell, { width: 110, marginLeft: 20 }]}>
          {moment(item.createdAt, "YYYY-MM-DDTHH:mm:ssZ").format("ddd MMM DD YYYY")}
          </Text>
          <Text style={[styles.cell, { width: 175, paddingLeft: 40}]}>{item.manufactors}</Text>
